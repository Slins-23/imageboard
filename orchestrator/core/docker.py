...
def run(name: str, image: str, network: str = "host", mounts: dict[Path, Path] | None = None) -> int:
    log.info(f"Running docker container '{name}' using image '{image}' and network '{network}'...", "docker")
    ...
    if mounts is not None:
        for host_path, name_path in mounts.items():
            if log.cfg.debug:
                log.info(f"Mount detected for name '{name}' - Host: '{host_path}' | name: '{name_path}'", "docker")
...
def rm(name: str) -> int:
    log.info(f"Removing docker container {name}...", "docker")
...
def restart(name: str) -> int:
    log.info(f"Restarting docker container {name}...", "docker")
